...
def ajaxAttendanceGroupList(request):
    context = {}
    context['attendance_groups'] = SpAttendanceGroups.objects.all()
...
def editAttendanceGroup(request,attendance_group_id):
    if request.method == "POST":
        ...
        context = {}
        context['attendance_group'] = SpAttendanceGroups.objects.get(pk=attendance_group_id)
...
def updateAttendanceGroupStatus(request,attendance_group_id):
    context = {}
    if SpAttendanceGroups.objects.filter(id=attendance_group_id).exists():
        attendance_group = SpAttendanceGroups.objects.get(id=attendance_group_id)
    ...
    if request.method == "POST":
        ...
        if TblClRoom.objects.filter(room=request.POST['room']).exists():
            ...
            collegename_path = getModelColumnById(
                SpOrganizations, request.POST['college_id'], 'organization_name')
            roots = settings.MEDIA_ROOT+'/documents/'+collegename_path+'/'+request.POST['room']
            room.path=roots 
            room.save()
            ...
            try:
                ...
                if e.errno != errno.EEXIST:
                    ...
                    print(e)
            roompath = settings.MEDIA_ROOT+'/documents/'+college_name+'/'
...
def addAlmirah(request):
    if request.method == "POST":
        response = {}
        if TblClAlmirah.objects.filter(almirah=clean_data(request.POST['almirah'])).exists():
            response['flag'] = False
            response['message'] = "Already exists."
        else:
            ...
            roots = settings.MEDIA_ROOT+'/documents/'+collegename_path+'/'+room_path+'/'+request.POST['almirah']
            almirah.path=roots 
            almirah.save()
            ...
            path = settings.MEDIA_ROOT+'/documents/'+college_name+ '/'+room_name+'/'
...
def addRack(request):
    if request.method == "POST":
        response = {}
        if TblClRack.objects.filter(rack=clean_data(request.POST['rack'])).exists():
            response['flag'] = False
            ...
            roots = settings.MEDIA_ROOT+'/documents/'+collegename_path+'/'+room_path+'/'+almira_path+'/'+request.POST['rack']
            rack.path=roots
            rack.save()
            ...
            college_name = getModelColumnById(
                SpOrganizations, request.POST['college_id'], 'organization_name')
            room_name = getModelColumnById(TblClRoom, request.POST['room_id'], 'room')
            path = settings.MEDIA_ROOT+'/documents/'+college_name+ '/'+room_name+'/'+almirah_name+'/'
...
def getCollegeByRoom(request):
    ...
    response = {}
    options = ''
    Room_no = TblClRoom.objects.filter(college_id=college_id)
...
def getRoomByAlmirah(request):
    ...
    response = {}
    options = ''
    Room_no = TblClAlmirah.objects.filter(room_id=room_id)
    for RoomNo in Room_no :
